...
def totallng(data, types):
    expectation = {s: {t: {f: [] for f in types['morphing']}
                       for t in types['technique']}for s in types['sex']}
            ...
            for kf, vf in vt.items():
                for sm in ['AN-', 'FE-', 'SA-']:  # 顔始点はAN, FE, SA
                    if sm in kf:
                        ...
                        break
    values_mean = {t: {f: 0 for f in types_st_morphing}
                   for t in types['technique']}
    values_median = {t: {f: 0 for f in types_st_morphing}
                     for t in types['technique']}
...
def diffReverseAspect(data, types): # 逆モーフィングとの比較．
    ...
    values_mean = {t: {f: 0 for f in types['morphing']}
                   for t in types['technique']}
    values_median = {
        t: {f: 0 for f in types['morphing']} for t in types['technique']}
        ...
        for kt, vt in values_pair.items():
            ...
            for i, ex in enumerate(vt):
                ...
                pos = np.arange(len(ex)) - totoal_width * \
                    (1 - (2*i+1)/len(vt))/2
                plt.bar(pos, ex.values(), width=totoal_width /
                        len(vt), label=st)
            plt.legend()
...
def tatisticalTest(data, types): # 検定
    ...
    result = stats.mstats.kruskal(
        all_ans['liner'], all_ans['stylegan'], all_ans['semanticstylegan'])
    ...
    for s in ['stylegan', 'liner']:
        result = stats.mannwhitneyu(
            all_ans['semanticstylegan'], all_ans[s])  # 片側検定でも良い．
